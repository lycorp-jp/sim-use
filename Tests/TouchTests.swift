...
struct TouchTests {
    ...
    func basicTouchDownUp() async throws {
        ...
        // Act - Touch down
        try await TestHelpers.runSimUseCommand("touch -x 200 -y 400 --down", simulatorUDID: defaultSimulatorUDID)
        ...
        // Act - Touch up
        try await TestHelpers.runSimUseCommand("touch -x 200 -y 400 --up", simulatorUDID: defaultSimulatorUDID)
        try await Task.sleep(nanoseconds: 500_000_000)
    ...
    func touchMove() async throws {
        ...
        try await TestHelpers.runSimUseCommand("touch -x 100 -y 300 --down", simulatorUDID: defaultSimulatorUDID)
        try await Task.sleep(nanoseconds: 300_000_000)
        
        try await TestHelpers.runSimUseCommand("touch -x 200 -y 400 --down", simulatorUDID: defaultSimulatorUDID)
        try await Task.sleep(nanoseconds: 300_000_000)
        
        try await TestHelpers.runSimUseCommand("touch -x 300 -y 500 --down", simulatorUDID: defaultSimulatorUDID)
        try await Task.sleep(nanoseconds: 300_000_000)
        
        // Touch up at final position
        try await TestHelpers.runSimUseCommand("touch -x 300 -y 500 --up", simulatorUDID: defaultSimulatorUDID)
        try await Task.sleep(nanoseconds: 300_000_000)
    ...
    func multipleTouchSequences() async throws {
        ...
        for i in 1...3 {
            ...
            // Touch down
            try await TestHelpers.runSimUseCommand("touch -x \(x) -y \(y) --down", simulatorUDID: defaultSimulatorUDID)
            try await Task.sleep(nanoseconds: 200_000_000)
            
            // Touch up
            try await TestHelpers.runSimUseCommand("touch -x \(x) -y \(y) --up", simulatorUDID: defaultSimulatorUDID)
            try await Task.sleep(nanoseconds: 200_000_000)
        }
    ...
    func touchDragSimulation() async throws {
        ...
        try await TestHelpers.runSimUseCommand("touch -x 100 -y 400 --down", simulatorUDID: defaultSimulatorUDID)
        try await Task.sleep(nanoseconds: 200_000_000)
        
        // Move in steps to simulate smooth drag (using touch down at each position)
        for x in stride(from: 150, through: 300, by: 50) {
            try await TestHelpers.runSimUseCommand("touch -x \(x) -y 400 --down", simulatorUDID: defaultSimulatorUDID)
            try await Task.sleep(nanoseconds: 100_000_000)
        }
        
        try await TestHelpers.runSimUseCommand("touch -x 300 -y 400 --up", simulatorUDID: defaultSimulatorUDID)
        try await Task.sleep(nanoseconds: 300_000_000)
    ...
    func touchWithDelays() async throws {
        ...
        let startTime = Date()
        try await TestHelpers.runSimUseCommand("touch -x 200 -y 300 --down --up --delay 1.0", simulatorUDID: defaultSimulatorUDID)
        let endTime = Date()
    ...
    func touchDelayTriggersLongPressRecognition() async throws {
        ...
        // Act
        try await TestHelpers.runSimUseCommand("touch -x 220 -y 340 --down --up --delay 1.0", simulatorUDID: defaultSimulatorUDID)
        try await Task.sleep(nanoseconds: 500_000_000)

...
struct SwipeTests {
    ...
    func basicSwipe() async throws {
        ...
        // Act
        try await TestHelpers.runSimUseCommand("swipe --start-x 100 --start-y 400 --end-x 300 --end-y 400", simulatorUDID: defaultSimulatorUDID)
    ...
    func swipeWithDuration() async throws {
        ...
        let startTime = Date()
        try await TestHelpers.runSimUseCommand("swipe --start-x 100 --start-y 400 --end-x 300 --end-y 400 --duration 2", simulatorUDID: defaultSimulatorUDID)
        let endTime = Date()
    ...
    func multipleSwipes() async throws {
        ...
        for i in 1...swipeCount {
            try await TestHelpers.runSimUseCommand("swipe --start-x \(100 + i * 30) --start-y \(400 + i * 20) --end-x \(200 + i * 30) --end-y \(400 + i * 20)", simulatorUDID: defaultSimulatorUDID)
